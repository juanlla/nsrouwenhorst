...
function rhmat(p, N)
    ...
    if N == 2
        ...
        # Step 2(b): if the number of statses N>2, apply the Rouwenhorst recursion to obtain the transiton matrix trans

        for i in 2:(N-1)
            P2 = p * [P1 zeros(size(P1,1),1); zeros(1,size(P1,2)) 0 ] + (1-p) * [zeros(size(P1,1),1) P1; 0 zeros(1,size(P1,2)) ] + (1-p) * [zeros(1,size(P1,2)) 0 ; P1 zeros(size(P1,1),1)] + p *     [0 zeros(1,size(P1,2)) ; zeros(size(P1,1),1) P1]
...
function nsrouwenhorst(rho, sigma_eps, N, T)
    ...
    if N < 2
        error("The state space has to have dimension N>1. Exiting")
    end

    if T < 2
        error("The time horizon has to have dimension N>1. Exiting")
    end
